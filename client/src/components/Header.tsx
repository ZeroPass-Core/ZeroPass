...
import zynLogo from "/ZeroPass-logo.png";
...
export default function Header() {
  ...
  const getNavHref = (subdomain: string, path: string) => {
    ...
    return `https://${subdomain}.ZeroPass.io`;
  };
  ...
  const handleLogoClick = () => {
    ...
    if (isProduction) {
      // If on any subdomain (contains dot before ZeroPass.io) OR is explicitly a known subdomain
      const isSubdomain = hostname !== 'ZeroPass.io' && hostname.endsWith('ZeroPass.io');
      
      if (isSubdomain) {
        // Redirect to main domain from ANY subdomain
        window.location.href = 'https://ZeroPass.io';
      } else {
      ...
      <nav className="flex items-center justify-between max-w-7xl mx-auto">
        <div onClick={handleLogoClick} className="flex items-center gap-3 cursor-pointer">
          <div className="w-10 h-10 rounded-md bg-black border border-white/10 flex items-center justify-center group relative overflow-hidden">
            <img src={zynLogo} alt="ZeroPass Logo" className="w-full h-full object-contain p-1" />
          </div>
          <div className="hidden sm:block">
            <span className="font-bold tracking-tight text-white text-lg">ZeroPass</span>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <TabItem label="Roadmap" id="roadmap" active={activeTab==='roadmap'} href={isDevelopment ? '/docs/roadmap' : 'https://docs.ZeroPass.io/roadmap'} />
        </div>
        ...
        {open && (
          ...
          >
            <div className="flex flex-col gap-2 pt-4">
              ...
              <MobileLink label="Roadmap" href={isDevelopment ? '/docs/roadmap' : 'https://docs.ZeroPass.io/roadmap'} />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
